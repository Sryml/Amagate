...
class OT_AddComponent(bpy.types.Operator):
    ...
    @staticmethod
    def auto_component(
        context: Context, active_object: Object | None = None, options=set()
    ):
        if active_object is None:
            active_object = context.active_object
        if not active_object:
        ...
        has_crush = next(
            (
                ...
                for i in active_object.children
                if i.type == "EMPTY"
                and i.name.startswith("Blade_Anchor_Crush")
                and i.visible_get()
            ),
            0,
        )
        ...
        bm.transform(object_matrix)
        bm.transform(Matrix.LocRotScale(None, rotation, None))
        ref_loc = rotation @ anchor_loc
        start_z = ref_loc.z - 0.3  # 距1H_R 30cm的位置计算X边界
        # 边界框
        min_x = float("inf")
        max_x = float("-inf")
        min_y = float("inf")
        ...
        for vert in bm.verts:
            co = vert.co
            if co.x < min_x and co.z < start_z:
                min_x = co.x
            elif co.x > max_x and co.z < start_z:
                max_x = co.x
            if co.y < min_y:
                min_y = co.y
            elif co.y > max_y:
            ...
            if co.z < min_z:
                ...
                max_z = co.z
        min_x = min_x if math.isfinite(min_x) else 0
        max_x = max_x if math.isfinite(max_x) else 0
        ...
        anchor2end = abs((min_co - anchor_loc).dot(anchor_z))  # type: ignore
        ...
        filepath = os.path.join(data.ADDON_PATH, "bin/ent_component.dat")
        mesh_dict = pickle.load(open(filepath, "rb"))
        only_trail = "only_trail" in options

        # 边缘
        edge_length = max(0.05, anchor2end - 0.1)
        edge_width = max(abs(max_x - min_x) * 0.5, 0.06)
        ...
        if not only_trail:
            ...
            for i in range(6):
                ...
                # rot = Quaternion()
                if i != 0:
                    rot = Matrix.Rotation(math.radians(120), 4, anchor_z)  # type: ignore
                    offset_x = rot @ offset_x
                    bmesh.ops.rotate(bm, cent=(0, 0, 0), matrix=rot, verts=bm.verts)  # type: ignore
                if i == 3:
                    offset_x = Vector((0, 0, 0))
                    # loc = -anchor_x * edge_width * 0.5
                    # bm.transform(Matrix.Translation(loc)) # type: ignore
                    q1 = Quaternion(anchor_x, math.radians(180))
                    q2 = Quaternion(anchor_y, math.radians(15))
                    matrix = (q2 @ q1).to_matrix().to_4x4()
                    bm.transform(matrix)
                    ...
                    # bmesh.ops.rotate(bm, cent=(0, 0, 0), matrix=rot, verts=bm.verts)  # type: ignore
                    # rot = Matrix.Rotation(math.radians(30), 4, anchor_y)  # type: ignore
                    # bmesh.ops.rotate(bm, cent=(0, 0, 0), matrix=rot, verts=bm.verts)  # type: ignore
        ...
        if has_crush:
            ...
            trail_length = min(0.90, edge_length)
            direction = 1
        half = anchor_z * trail_length * 0.5
        offset = (max_length / 2 - trail_length * 0.5) * anchor_z
        bm = bmesh.new()
        bm.verts.new(half * direction)
        bm.verts.new(-half * direction)
        bm.edges.new(bm.verts)

        obj_name = "Blade_Trail_1"
        mesh = bpy.data.meshes.new(obj_name)
        obj = bpy.data.objects.new(obj_name, mesh)  # type: Object # type: ignore
        obj.show_in_front = True
        ...
        obj.location = anchor_center + offset
        bm.to_mesh(mesh)
        obj.parent = active_object
        obj.matrix_parent_inverse = object_matrix.inverted()
        ...
        if not only_trail:
            ...
            # spike_length = (anchor_z * edge_length * 0.25).length
            spike_length = max(0.05, anchor2end - 0.3)
            bm = bmesh.new()
            verts = []
            for co in mesh_data["vertices"]:
                verts.append(bm.verts.new(co))
            for idx in mesh_data["edges"]:
                bm.edges.new([verts[i] for i in idx])
            offset = (max_length / 2 - spike_length) * anchor_z
            # 调整大小
            move_z = spike_length - 0.1
    ...
    def execute(self, context: Context):
        ...
        for filepath in files:
            ...
            for obj in edges + spikes + trails:
                bpy.data.objects.remove(obj)
            OT_AddComponent.auto_component(context, entity, options=options)
            # 导出
            ...
            entity.select_set(False)
            context.view_layer.active_layer_collection.hide_viewport = True
            count += 1
        #
        logger.info(f"Ignored {ignore_count} files, Rebuilt {count} files")
        self.report({"INFO"}, f"Ignored {ignore_count} files, Rebuilt {count} files")
...
class OT_EntityNote(bpy.types.Operator):
    ...
    def draw_menu(this, context: Context):
        ...
        text = (
            ...
            pgettext("In the game, Trail will move towards vertex 0"),
            pgettext(
    ...
    def import_bod(context: Context, filepath):
        ...
        with open(filepath, "rb") as f:
            ...
            for idx in range(num):
                ...
                #
                edge_count += 1
                obj_name = f"Blade_Edge_{edge_count}"
                mesh = bpy.data.meshes.new(obj_name)
                obj = bpy.data.objects.new(
                    obj_name, mesh
                )  # type: Object # type: ignore
            ...
            for idx in range(num):
                ...
                #
                spike_count += 1
                obj_name = f"Blade_Spike_{spike_count}"
                mesh = bpy.data.meshes.new(obj_name)
                obj = bpy.data.objects.new(
                    obj_name, mesh
                )  # type: Object # type: ignore
            ...
            for idx in range(num):
                ...
                #
                trail_count += 1
                obj_name = f"Blade_Trail_{trail_count}"
                mesh = bpy.data.meshes.new(obj_name)
                obj = bpy.data.objects.new(
                    obj_name, mesh
                )  # type: Object # type: ignore
...
# 导出实体
class OT_ExportEntity(bpy.types.Operator):
    bl_idname = "amagate.export_entity"
    bl_label = "Export Entity"
    bl_description = "Export Entity"
    bl_options = {"INTERNAL"}
    ...
    action: EnumProperty(
        ...
        description="",
        items=[
            ("0", "Export to temporary file (Selected Only)", ""),
        ],
        options={"HIDDEN"},
    )  # type: ignore
    ...
    def execute(self, context: Context):
        ...
        scene_data = context.scene.amagate_data
        coll = L3D_data.ensure_collection(L3D_data.E_COLL)
        entities = []  # type: list[entity_data.EntityProperty]
        selected_only = True if not self.main and self.action == "0" else False
        for ent in coll.all_objects:
            if selected_only and not ent.select_get():
                continue
            if not ent.amagate_data.is_entity:
                continue
            ent_data = ent.amagate_data.get_entity_data()
            if ent_data.Name not in scene_data["EntityManage"]:
                continue
            #
        ...
        #
        CombustionData = []
        combustion_buffer = StringIO()
        head_buffer = StringIO()
        buffer = StringIO()
        #
        head_buffer.write("# Automatically generated by Amagate\n\n")
        head_buffer.write("import Bladex\n")
        head_buffer.write("import ItemTypes\n")
        head_buffer.write("import EnemyTypes\n")
        head_buffer.write("import Reference\n")
        head_buffer.write("import darfuncs\n")
        ...
        entities.sort(
            key=lambda x: (
                -2
                if x.Name == "Player1"
                else -1 if len(x.contained_item) != 0 else int(x.ObjType)
            )
        )
        ...
        def write_entity(ent_data: entity_data.EntityProperty):
            for coll_prop in (
                ...
                for item in coll_prop:
                    ent_data2 = item.obj.amagate_data.get_entity_data()
                    write_entity(ent_data2)
                    if ent_data2 in entities:
                        entities.remove(ent_data2)
            #
            ...
            if ObjType == "None":
                W_ObjType = "''"
            elif ObjType != "Assigned By Script":
                W_ObjType = repr(ObjType)
            else:
                W_ObjType = f"Reference.ObjType({ent_data.Kind!r})"
            #
            ...
            for prop_name in ("Alpha", "SelfIlum", "Static", "CastShadows"):
                ...
                if value != properties[prop_name].default:  # type: ignore
                    buffer.write(
                        f"o.{prop_name} = {(int(value) if isinstance(value, bool) else round(value, 3))}\n"
                    )
            # 角色
            if ObjType == "Person":
                # 设置皮肤
                ...
                # 添加到库存
                for coll_prop in (ent_data.equipment_inv, ent_data.prop_inv):
                    for item in coll_prop:
                        buffer.write(
            ...
            # 灯光
            if ent_data.has_fire:
                buffer.write(f"o.FiresIntensity = [{ent_data.FiresIntensity}]\n")
            if ent_data.has_light:
                Intensity = round(ent_data.light_prop.Intensity, 2)
                Precision = round(ent_data.light_prop.Precision, 5)
                Color = tuple(math.ceil(i * 255) for i in ent_data.light_prop.Color)
                buffer.write(f"o.Lights = [({Intensity},{Precision},{Color})]\n")
                #
                sub_properties = ent_data.light_prop.bl_rna.properties
                get_light = False
                for prop_name in ("CastShadows", "Flick", "Visible"):
                    value = getattr(ent_data.light_prop, prop_name)
                    if value != sub_properties[prop_name].default:  # type: ignore
                        if not get_light:
                            buffer.write(f"light = AuxFuncs.GetSpot(o)\n")
                            get_light = True

                        buffer.write(f"light.{prop_name} = {int(value)}\n")
            #
            if ObjType == "Person":
                if ent_data.Name == "Player1":
                    if ent_data.Life_Enabled:
                        buffer.write(f"o.Life = {ent_data.Life}\n")
                    else:
                        buffer.write(
                ...
                if ent_data.instance_data:
                    buffer.write("ItemTypes.ItemDefaultFuncs(o)\n")
                # 火炬可用
                if ent_data.torch_usable:
                    buffer.write(
                        f"Torchs.SetUsable({ent_data.Name!r}, {ent_data.torch_light_int}, {ent_data.torch_fire_int}, {ent_data.torch_life})\n"
                    )

            # 可燃的
            if ent_data.Burnable:
                buffer.write(
                    f"Actions.SetBurnable({ent_data.Name!r}, {ent_data.BurnTime}, {ent_data.DestroyTimeAfterBurn})\n"
                )
                if ent_data.Kind not in CombustionData:
                    CombustionData.append(ent_data.Kind)
                    combustion_buffer.write(
                        f"Bladex.AddCombustionDataFor({ent_data.Kind!r}, 'LargeFire', 250, 400, 4, 1.5, 1, 600)\n"
                    )
                    prefix = f"{ent_data.Kind}Pieza".lower()
                    for item in entity_data.get_ent_enum(None, None):
                        if item[2].lower().startswith(prefix):
                            combustion_buffer.write(
                                f"Bladex.AddCombustionDataFor({item[2]!r}, 'Fire', 250, 400, 4, 1.5, 1, 60)\n"
                            )

            # 可破坏的
            if ent_data.Breakable:
                contained_item = (
                    ent_data.contained_item[0].obj.amagate_data.get_entity_data().Name
                    if len(ent_data.contained_item) != 0
                    else ""
                )
                buffer.write(
                    f"Breakings.SetBreakable({ent_data.Name!r}, {ent_data.PiecesDestroyTime}, {ent_data.DestroyTime}, {contained_item!r})\n"
                )
            # End
            buffer.write("\n")
        ...
        for ent_data in entities:
            buffer.write("#\n")
            write_entity(ent_data)
        #
        if selected_only:
            filename = "AG_ObjsTemp.py"
            mode = "a"
        else:
            filename = "AG_Objs.py"
            mode = "w"
        with open(map_dir / filename, mode, encoding="utf-8") as file:
            if selected_only:
                ...
                if file.tell() == 0:
                    file.write(head_buffer.getvalue())
                file.write(formatted)
            else:
                file.write(head_buffer.getvalue())
            #
            file.write(buffer.getvalue())
            file.write(combustion_buffer.getvalue())
            if CombustionData:
                file.write("\n")
        head_buffer.close()
        buffer.close()
